...
void iGameEnemy::ChangeState(int alId)
{
	...
	sprintf_s(sStr,"%s State %s -> %s",msName.c_str(),sStateName1.c_str(),sStateName2.c_str());
											  ...
											  cWorld3D *apWorld)
{
	...
	else if (sSubtype == "Soldier")
	{
		pEnemy = hplNew(cGameEnemy_Soldier, (mpInit, mpEntity->GetName(), pGameElem));
	}
	else if (sSubtype == "BullPig")
	{
		pEnemy = hplNew(cGameEnemy_BullPig, (mpInit, mpEntity->GetName(), pGameElem));
	}
#ifndef DEMO_VERSION
