...
bool CheckSupport(cInit *apInit)
{
	...
	*/
	...
	if(	pTestVtxProg->CreateFromFile("../redist/core/programs/Fallback01_Diffuse_Light_p1_vp.cg","main")==false)
	{
		Log("Did not succeed!\n");
...
tWString gsDefaultSettingsPath = _W("../redist/config/default_settings.cfg");
...
bool cInit::Init(tString asCommandLine)
{
	...
	if (FolderExists(sPersonalDir + PERSONAL_RELATIVEROOT _W("Hopelessness in War/HPL1"))
			&& !IsFileLink(sPersonalDir + PERSONAL_RELATIVEROOT _W("Hopelessness in War/HPL1"))) {
		// Create the new folder
		if(!FolderExists(sPersonalDir + PERSONAL_RELATIVEROOT _W("HopelessnessInWar")))
			CreateFolder(sPersonalDir + PERSONAL_RELATIVEROOT _W("HopelessnessInWar"));
		// Move the Older Episode 1 to the new Overture
		RenameFile(sPersonalDir + PERSONAL_RELATIVEROOT _W("Hopelessness in War/HPL1"),
				sPersonalDir + PERSONAL_RELATIVEROOT _W("HopelessnessInWar/HPL1"));
		// Link back the old one to the new one
		LinkFile(sPersonalDir + PERSONAL_RELATIVEROOT _W("HopelessnessInWar/HPL1"),
			sPersonalDir + PERSONAL_RELATIVEROOT _W("Hopelessness in War/HPL1"));
	}
	#endif
    ...
    mpGameConfig = hplNew( cConfigFile, (_W("../redist/config/game.cfg")) );
	mpGameConfig->Load();
	...
	msStartMap = mpConfig->GetString("Map","File","level00_00_trench01.dae");
	msStartLink = mpConfig->GetString("Map","StartPos","trench_spot01");
	...
	// RESOURCE INIT /////////////////////
	mpGame->GetResources()->LoadResourceDirsFile("../redist/resources.cfg"); // Just a little bit of "../redist/"!
	...
	// PHYSICS INIT /////////////////////
	mpGame->GetPhysics()->LoadSurfaceData("../redist/materials.cfg", mpGame->GetHaptic());
	...
	mpGame->GetGraphics()->GetRenderer3D()->SetRefractionUsed(mpConfig->GetBool("Graphics", "Refractions", true));
