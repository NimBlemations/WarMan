...
iGameEnemyState_BullPig_Base::iGameEnemyState_BullPig_Base(int alId, cInit* apInit, iGameEnemy* apEnemy)
	: iGameEnemyState(alId, apInit, apEnemy)
{
	mpEnemyBullPig = static_cast<cGameEnemy_BullPig*>(mpEnemy);
}

//-----------------------------------------------------------------------

void iGameEnemyState_BullPig_Base::OnSeePlayer(const cVector3f& avPosition, float afChance)
{
	//return;
	if (mpPlayer->GetHealth() <= 0) return;

	if (afChance >= mpEnemyBullPig->mfIdleMinSeeChance)
	{
		/*if( (mlId == STATE_IDLE || mlId == STATE_INVESTIGATE || mlId == STATE_PATROL) &&
			cMath::RandRectf(0,1) < mpEnemyBullPig->mfIdleCallBackupChance &&
...
bool iGameEnemyState_BullPig_Base::OnHearNoise(const cVector3f& avPosition, float afVolume)
{
	//return false;
	float afDistance = (mpMover->GetCharBody()->GetPosition() - avPosition).Length();

	if (afVolume >= mpEnemyBullPig->mfIdleMinHearVolume && afDistance > 0.4f)
	{
...
void iGameEnemyState_BullPig_Base::OnTakeHit(float afDamage)
{
	if (afDamage >= mpEnemyBullPig->mfMinKnockDamage)
	{
		if (mpInit->mbWeaponAttacking)
		{
			float fChance = afDamage / mpEnemyBullPig->mfCertainKnockDamage;//(mpEnemyBullPig->mfCertainKnockDamage*4);
		...
		{
			if (afDamage >= mpEnemyBullPig->mfCertainKnockDamage)
			{
				mpEnemy->ChangeState(STATE_KNOCKDOWN);
			}
			else
			{
				float fChance = afDamage / mpEnemyBullPig->mfCertainKnockDamage;
				if (fChance > cMath::RandRectf(0, 1))
				{
					mpEnemy->ChangeState(STATE_KNOCKDOWN);
				}
...
void iGameEnemyState_BullPig_Base::OnFlashlight(const cVector3f& avPosition)
{
...
void iGameEnemyState_BullPig_Base::OnDeath(float afDamage)
{
...
void cGameEnemyState_BullPig_Idle::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	...
	//Setup enemy
	mpEnemy->SetFOV(mpEnemyBullPig->mfIdleFOV);
...
void cGameEnemyState_BullPig_Idle::OnLeaveState(iGameEnemyState* apNextState)
{
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Idle::OnUpdate(float afTimeStep)
{
...
void cGameEnemyState_BullPig_Patrol::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->UseMoveStateAnimations();
	...
	mfIdleSoundTime = cMath::RandRectf(mpEnemyBullPig->mfIdleSoundMinInteraval,
		mpEnemyBullPig->mfIdleSoundMaxInteraval);
...
void cGameEnemyState_BullPig_Patrol::OnLeaveState(iGameEnemyState* apNextState)
{
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Patrol::OnUpdate(float afTimeStep)
{
	/////////////////////////////////
	...
	if (mbWaiting)
	{
		...
		if (mfIdleSoundTime <= 0)
		{
			...
			mpEnemy->PlaySound(mpEnemyBullPig->msIdleSound);
		}
		else
		{
...
void cGameEnemyState_BullPig_Patrol::OnAnimationOver(const tString& asName)
{
	mbAnimation = false;
}
...
void cGameEnemyState_BullPig_Attention::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->PlayAnim("Attention", true, 0.2f);
	...
	mpEnemy->PlaySound(mpEnemyBullPig->msAttentionSound);
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Attention::OnLeaveState(iGameEnemyState* apNextState)
{
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Attention::OnUpdate(float afTimeStep)
{
	mpMover->TurnToPos(mpPlayer->GetCharacterBody()->GetFeetPosition());
...
void cGameEnemyState_BullPig_Attention::OnSeePlayer(const cVector3f& avPosition, float afChance)
{

}
bool cGameEnemyState_BullPig_Attention::OnHearNoise(const cVector3f& avPosition, float afVolume)
{
	return false;
}
void cGameEnemyState_BullPig_Attention::OnFlashlight(const cVector3f& avPosition)
{
	mpEnemy->ChangeState(STATE_HUNT);
}

void cGameEnemyState_BullPig_Attention::OnAnimationOver(const tString& asAnimName) // They WILL come right after you!
{
	if (asAnimName == "Attention")
	{
		mpEnemy->ChangeState(STATE_HUNT);
		mpEnemy->PlaySound(mpEnemyBullPig->msIdleFoundPlayerSound);
	}
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Attention::OnDraw()
{
...
void cGameEnemyState_BullPig_Eat::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	...
	//Setup enemy
	mpEnemy->SetFOV(mpEnemyBullPig->mfEatFOV);
...
void cGameEnemyState_BullPig_Eat::OnLeaveState(iGameEnemyState* apNextState)
{
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Eat::OnUpdate(float afTimeStep)
{
	mfTime -= afTimeStep;
	if (mfTime <= 0)
	{
...
bool cGameEnemyState_BullPig_Eat::OnHearNoise(const cVector3f& avPosition, float afVolume)
{
	//return false;
	if (afVolume >= mpEnemyBullPig->mfEatMinHearVolume)
	{
		mpEnemy->SetTempPosition(avPosition);
		mpEnemy->ChangeState(STATE_INVESTIGATE);
		return true;
...
void cGameEnemyState_BullPig_Eat::OnSeePlayer(const cVector3f& avPosition, float afChance)
{
	//return;
	if (mpPlayer->GetHealth() <= 0) return;

	if (afChance >= mpEnemyBullPig->mfEatMinSeeChance)
	{
		mpEnemy->ChangeState(STATE_HUNT);
		mpEnemyBullPig->PlaySound(mpEnemyBullPig->msIdleFoundPlayerSound);
	}
...
void cGameEnemyState_BullPig_Investigate::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->UseMoveStateAnimations();
	...
	//Play sound
	mpEnemy->PlaySound(mpEnemyBullPig->msInvestigateSound);
...
void cGameEnemyState_BullPig_Investigate::OnLeaveState(iGameEnemyState* apNextState)
{
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Investigate::OnUpdate(float afTimeStep)
{
...
bool cGameEnemyState_BullPig_Investigate::OnHearNoise(const cVector3f& avPosition, float afVolume)
{
	if (mfHearSoundCount <= 0 && mfHighestVolume < afVolume &&
		afVolume >= mpEnemyBullPig->mfIdleMinHearVolume)
	{
...
void cGameEnemyState_BullPig_MoveTo::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->UseMoveStateAnimations();
...
void cGameEnemyState_BullPig_MoveTo::OnLeaveState(iGameEnemyState* apNextState)
{
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_MoveTo::OnUpdate(float afTimeStep)
{
	////////////////////////////////
	//Play idle sound
	if (mfIdleSoundTime <= 0)
...
void cGameEnemyState_BullPig_Hunt::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->UseMoveStateAnimations();
	...
	if (mpInit->mDifficulty == eGameDifficulty_Easy)
		mpMover->GetCharBody()->SetMaxPositiveMoveSpeed(eCharDir_Forward, mpEnemyBullPig->mfHuntSpeed * 0.7f * fMul);
	else if (mpInit->mDifficulty == eGameDifficulty_Normal)
		mpMover->GetCharBody()->SetMaxPositiveMoveSpeed(eCharDir_Forward, mpEnemyBullPig->mfHuntSpeed * fMul);
	else
		mpMover->GetCharBody()->SetMaxPositiveMoveSpeed(eCharDir_Forward, mpEnemyBullPig->mfHuntSpeed * 1.25f * fMul);
#else
	mpMover->GetCharBody()->SetMaxPositiveMoveSpeed(eCharDir_Forward, mpEnemyBullPig->mfHuntSpeed);
#endif


	//Setup enemy
	mpEnemy->SetFOV(mpEnemyBullPig->mfHuntFOV);
	...
	mbLostPlayer = false;
	mfLostPlayerCount = 0;
	mfMaxLostPlayerCount = mpEnemyBullPig->mfHuntForLostPlayerTime;
...
void cGameEnemyState_BullPig_Hunt::OnLeaveState(iGameEnemyState* apNextState)
{
	mpMover->SetMaxDoorToughness(-1);
}
...
void cGameEnemyState_BullPig_Hunt::OnUpdate(float afTimeStep)
{
	...
	if (mbFreePlayerPath || (mbFoundNoPath && mpMover->IsMoving() == false))
	{
		...
		if (mpMover->DistanceToChar2D(mpPlayer->GetCharacterBody()) < mpEnemyBullPig->mfAttackDistance)
		{
			float fHeight = mpMover->GetCharBody()->GetPosition().y -
				mpPlayer->GetCharacterBody()->GetPosition().y;
			...
			if (fHeight < 0)
			{
				...
				float fMax = mpEnemyBullPig->mvAttackDamageSize.y;///2.0f;
				if (fHeight > -fMax)
				{
...
void cGameEnemyState_BullPig_Hunt::OnSeePlayer(const cVector3f& avPosition, float afChance)
{
	if (mbLostPlayer && afChance >= mpEnemyBullPig->mfHuntMinSeeChance)
	{
		mbLostPlayer = false;
		mfUpdatePathCount = 0;
	}
...
bool cGameEnemyState_BullPig_Hunt::OnHearNoise(const cVector3f& avPosition, float afVolume)
{
	//////////////////////////////////
	...
	if (mbLostPlayer)
	{
		...
		if (afVolume >= mpEnemyBullPig->mfHuntMinHearVolume)
		{
...
void cGameEnemyState_BullPig_Hunt::OnDraw()
{
	...
	mpInit->mpDefaultFont->Draw(cVector3f(0, 110, 100), 14, cColor(1, 1, 1, 1), eFontAlign_Left,
		...
		mlStuckAtMaxCount,
		fDist,
		mpEnemyBullPig->mfAttackDistance);
}
...
void cGameEnemyState_BullPig_Attack::OnEnterState(iGameEnemyState* apPrevState)
{
	///////////////
	//Setup body
	mpEnemy->SetupBody();
	if (mpEnemyBullPig->mfAttackSpeed > 0)
	{
		mpMover->GetCharBody()->SetMaxPositiveMoveSpeed(eCharDir_Forward, mpEnemyBullPig->mfAttackSpeed);
	...
	///////////////
	//Other
	mpEnemyBullPig->PlaySound(mpEnemyBullPig->msAttackStartSound);

	mfDamageTimer = mpEnemyBullPig->mfAttackDamageTime;
	mfJumpTimer = mpEnemyBullPig->mfAttackJumpTime;
	mbAttacked = false;
...
void cGameEnemyState_BullPig_Attack::OnLeaveState(iGameEnemyState* apNextState)
{
	mpEnemyBullPig->SetSkipSoundTriggerCount(2.0f);
	mpMover->ResetStuckCounter();
...
void cGameEnemyState_BullPig_Attack::OnUpdate(float afTimeStep)
{
	//Move forward
	if (mpEnemyBullPig->mfAttackSpeed > 0)
	{
	...
	float fMinRange = mpEnemyBullPig->mfAttackDamageRange;
	...
	if (fDist2D <= fMinRange * fMinRange && mfDamageTimer <= 0)
	{
		if (mbAttacked == false)
		{
			cVector3f vPos = mpMover->GetCharBody()->GetPosition() +
				...
				mpEnemyBullPig->mfAttackDamageRange;
			...
			if (mpInit->mpAttackHandler->CreateShapeAttack(mpEnemyBullPig->GetAttackShape(),
				mtxOffset,
				mpMover->GetCharBody()->GetPosition(),
				cMath::RandRectf(mpEnemyBullPig->mfAttackMinDamage,
					mpEnemyBullPig->mfAttackMaxDamage),

				mpEnemyBullPig->mfAttackMinMass, mpEnemyBullPig->mfAttackMaxMass,
				mpEnemyBullPig->mfAttackMinImpulse, mpEnemyBullPig->mfAttackMaxImpulse,

				mpEnemyBullPig->mlAttackStrength,

				target, NULL))
			{
				mpEnemyBullPig->PlaySound(mpEnemyBullPig->msAttackHitSound);
			}
...
void cGameEnemyState_BullPig_Attack::OnAnimationOver(const tString& asName)
{
	if (mpPlayer->GetHealth() <= 0)
	{
...
void cGameEnemyState_BullPig_Attack::OnPostSceneDraw()
{
	...
	cVector3f vSize = mpEnemyBullPig->GetAttackShape()->GetSize();
...
void cGameEnemyState_BullPig_Flee::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->UseMoveStateAnimations();
	...
	mpEnemy->SetupBody();
	mpMover->GetCharBody()->SetMaxPositiveMoveSpeed(eCharDir_Forward, mpEnemyBullPig->mfHuntSpeed);
	mpMover->GetCharBody()->SetMaxNegativeMoveSpeed(eCharDir_Forward, -mpEnemyBullPig->mfFleeBackSpeed);
	...
	if (mbBackingFromBreakDoor)
	{
		...
		mfTimer = mpEnemyBullPig->mfFleeBackTime;
	...
	else
	{
		if ((apPrevState->GetId() == STATE_KNOCKDOWN || apPrevState->GetId() == STATE_HUNT ||
			cMath::RandRectf(0, 1) < 0)//mpEnemyBullPig->mfFleePositionChance) 
			)
		{
			cAINode* pNode = mpMover->GetAINodeInRange(mpEnemyBullPig->mfFleePositionMinDistance * fPosMul,
				mpEnemyBullPig->mfFleePositionMaxDistance * fPosMul);
			if (pNode)
			{
			...
			mfTimer = mpEnemyBullPig->mfFleePositionMaxTime;
			mbBackwards = false;
		}
		else if (cMath::RandRectf(0, 1) < mpEnemyBullPig->mfFleeBackChance)
		{
			mfBackAngle = mpMover->GetCharBody()->GetYaw();
			mbBackwards = true;
...
void cGameEnemyState_BullPig_Flee::OnLeaveState(iGameEnemyState* apNextState)
{
	mpMover->ResetStuckCounter();
}
...
void cGameEnemyState_BullPig_Flee::OnUpdate(float afTimeStep)
{
	mfTimer -= afTimeStep;
...
void cGameEnemyState_BullPig_BreakDoor::OnEnterState(iGameEnemyState* apPrevState)
{
	//Setup body
	mpEnemy->SetupBody();
	if (mpEnemyBullPig->mfBreakDoorSpeed > 0)
		mpMover->GetCharBody()->SetMaxPositiveMoveSpeed(eCharDir_Forward, mpEnemyBullPig->mfBreakDoorSpeed);


	//Animation
	mpEnemy->PlayAnim(mpEnemyBullPig->msBreakDoorAnimation, false, 0.2f);

	mpEnemyBullPig->PlaySound(mpEnemyBullPig->msBreakDoorStartSound);

	mfDamageTimer = mpEnemyBullPig->mfBreakDoorDamageTime;
	mfStopMoveTimer = mpEnemyBullPig->mfBreakDoorDamageTime + 1.1f;
	mbAttacked = false;
	mbStopped = false;
...
void cGameEnemyState_BullPig_BreakDoor::OnLeaveState(iGameEnemyState* apNextState)
{
	mpEnemyBullPig->SetSkipSoundTriggerCount(2.0f);
	mpMover->ResetStuckCounter();
}
...
void cGameEnemyState_BullPig_BreakDoor::OnUpdate(float afTimeStep)
{
	//Move forward
	if (mpEnemyBullPig->mfBreakDoorSpeed > 0 && mlCount == 0 && mbAttacked == false)
	{
		//Skip this for now
		//mpMover->GetCharBody()->Move(eCharDir_Forward,1.0f,afTimeStep);
	}
	...
	if (mfDamageTimer <= 0)
	{
		if (mbAttacked == false)
		{
			cVector3f vPos = mpMover->GetCharBody()->GetPosition() +
				mpMover->GetCharBody()->GetForward() *
				mpEnemyBullPig->mfBreakDoorDamageRange;
			...
			if (mpInit->mpAttackHandler->CreateShapeAttack(mpEnemyBullPig->GetBreakDoorShape(),
				mtxOffset,
				mpMover->GetCharBody()->GetPosition(),
				cMath::RandRectf(mpEnemyBullPig->mfBreakDoorMinDamage,
					mpEnemyBullPig->mfBreakDoorMaxDamage),

				mpEnemyBullPig->mfBreakDoorMinMass, mpEnemyBullPig->mfBreakDoorMaxMass,
				mpEnemyBullPig->mfBreakDoorMinImpulse, mpEnemyBullPig->mfBreakDoorMaxImpulse,

				mpEnemyBullPig->mlBreakDoorStrength,

				target, NULL))
			{
				mpEnemyBullPig->PlaySound(mpEnemyBullPig->msBreakDoorHitSound);
				...
				if (pDoor)
				{
					...
					if (pDoor->GetToughness() - mpEnemyBullPig->mlBreakDoorStrength >= 4)
					{
...
void cGameEnemyState_BullPig_BreakDoor::OnAnimationOver(const tString& asName)
{
	if (mlCount == 0)
	{
		if (mpEnemyBullPig->mbBreakDoorRiseAtEnd)
		{
...
void cGameEnemyState_BullPig_BreakDoor::OnPostSceneDraw()
{
	cCamera3D* pCamera = static_cast<cCamera3D*>(mpInit->mpGame->GetScene()->GetCamera());
	...
	cVector3f vSize = mpEnemyBullPig->GetBreakDoorShape()->GetSize();
	mpInit->mpGame->GetGraphics()->GetLowLevel()->DrawBoxMaxMin(vSize * 0.5f, vSize * -0.5f,
		cColor(1, 0, 1, 1));
}
...
void cGameEnemyState_BullPig_KnockDown::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->PlayAnim("Idle", true, 0.7f);

	//Sound
	mpEnemy->PlaySound(mpEnemyBullPig->msKnockDownSound);
...
void cGameEnemyState_BullPig_KnockDown::OnLeaveState(iGameEnemyState* apNextState)
{
...
void cGameEnemyState_BullPig_KnockDown::OnUpdate(float afTimeStep)
{
	if (mbCheckAnim)
	{
...
void cGameEnemyState_BullPig_KnockDown::OnAnimationOver(const tString& asName)
{
	iCharacterBody* pCharBody = mpEnemy->GetMover()->GetCharBody();
...
void cGameEnemyState_BullPig_Dead::OnEnterState(iGameEnemyState* apPrevState)
{
	//Animation
	mpEnemy->PlayAnim("Idle", true, 0.7f);
	...
	if (mpEnemy->IsLoading() == false)
		mpEnemy->PlaySound(mpEnemyBullPig->msDeathSound);
...
void cGameEnemyState_BullPig_Dead::OnLeaveState(iGameEnemyState* apNextState)
{
}

//-----------------------------------------------------------------------

void cGameEnemyState_BullPig_Dead::OnUpdate(float afTimeStep)
{
}
...
cGameEnemy_BullPig::cGameEnemy_BullPig(cInit* apInit, const tString& asName, TiXmlElement* apGameElem) : iGameEnemy(apInit, asName, apGameElem)
{
	...
	mfLengthBodyToAss = cString::ToFloat(apGameElem->Attribute("LengthBodyToAss"), 2.0f);
	...
	//////////////////////////////
	//Set up states
	AddState(hplNew(cGameEnemyState_BullPig_Idle, (STATE_IDLE, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Hunt, (STATE_HUNT, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Attack, (STATE_ATTACK, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Flee, (STATE_FLEE, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_KnockDown, (STATE_KNOCKDOWN, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Dead, (STATE_DEAD, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Patrol, (STATE_PATROL, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Investigate, (STATE_INVESTIGATE, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_BreakDoor, (STATE_BREAKDOOR, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_MoveTo, (STATE_MOVETO, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Eat, (STATE_EAT, mpInit, this)));
	AddState(hplNew(cGameEnemyState_BullPig_Attention, (STATE_ATTENTION, mpInit, this)));
}

//-----------------------------------------------------------------------

cGameEnemy_BullPig::~cGameEnemy_BullPig()
{
...
void cGameEnemy_BullPig::OnLoad()
{
	//Create attack shape
...
void cGameEnemy_BullPig::OnUpdate(float afTimeStep)
{
...
void cGameEnemy_BullPig::ShowPlayer(const cVector3f& avPlayerFeetPos)
{
...
bool cGameEnemy_BullPig::MoveToPos(const cVector3f& avFeetPos)
{
	if (mlCurrentState == STATE_IDLE || mlCurrentState == STATE_PATROL)
	{
...
bool cGameEnemy_BullPig::IsFighting()
{
