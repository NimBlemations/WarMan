...
public:
	...
	void OnMouseDown(eMButton aButton)
	{
		...
		char sTempVec[256];
		sprintf_s(sTempVec,"%.1f",mpInit->mpButtonHandler->mfMouseSensitivity);
...
public:
	...
	void OnMouseDown(eMButton aButton)
	{
		...
		char sTempVec[256];
		sprintf_s(sTempVec,"%.0f",mpInit->mpGame->GetSound()->GetLowLevel()->GetVolume()*100);
...
tString gvTextureFilter[] = {"Bilinear","Trilinear","Nearest"};
...
public:
	...
	void OnMouseDown(eMButton aButton)
	{
		...
		char sTempVec[256];
		sprintf_s(sTempVec,"%d x %d",mpInit->mvScreenSize.x, mpInit->mvScreenSize.y);
...
void cMainMenu::CreateWidgets()
{
	...
	if(bFirstStart)
	{
		...
		vPos.x = 405;
		sprintf_s(sTempVec,"%.1f",mpInit->mpGame->GetGraphics()->GetLowLevel()->GetGammaCorrection());
	...
	if(mpInit->mbHapticsAvailable)
	{
		...
		vPos.y += 29;
		sprintf_s(sTempVec,"%.1f",mpInit->mfHapticForceMul);
		sText = cString::To16Char(sTempVec);
		...
		vPos.y += 29;
		sprintf_s(sTempVec,"%.1f",mpInit->mpPlayer->GetHapticCamera()->GetInteractModeCameraSpeed());
		sText = cString::To16Char(sTempVec);
		...
		vPos.y += 29;
		sprintf_s(sTempVec,"%.1f",mpInit->mpPlayer->GetHapticCamera()->GetActionModeCameraSpeed());
		sText = cString::To16Char(sTempVec);
