...
#define WITHIN_THE_LINES(vCur, vMin) if (cMath::Abs(vCur.x) > vMin.x) {if (vCur.x < -vMin.x) vCur.x = -vMin.x; else if (vCur.x > vMin.x) vCur.x = vMin.x;} if (cMath::Abs(vCur.y) > vMin.y) {if (vCur.y < -vMin.y) vCur.y = -vMin.y; else if (vCur.y > vMin.y) vCur.y = vMin.y;} if (cMath::Abs(vCur.z) > vMin.z) {if (vCur.z < -vMin.z) vCur.z = -vMin.z; else if (vCur.z > vMin.z) vCur.z = vMin.z;}
...
void cPlayerState_Move::OnUpdate(float afTimeStep)
{
	...
	cVector3f vBodyCentre = cMath::MatrixMul(mpPushBody->GetLocalMatrix(), mpPushBody->GetMassCentre());
	cVector3f vJointToBody = cMath::Vector3Normalize(vBodyCentre - mpPushBody->GetJoint(0)->GetPivotPoint());
	if (mpPushBody->GetEntityType() == "Object")
		vJointToBody += mpPushBody->GetJoint(0)->GetPinDir();
	//cVector3f vPosition = mpPushBody->GetJoint(0)->GetPivotPoint() + mpPushBody->GetJoint(0)->Get;
	//WITHIN_THE_LINES(vPosition, mpPushBody->GetShape()->GetSize())
	cVector3f vProjPos = cMath::MatrixMul(mpPlayer->GetCamera()->GetViewMatrix(),vBodyCentre); // Usually mvPickPoint
...
{
	cVector3f vForce = (mpPlayer->GetCharacterBody()->GetForward() * (afMul * 4.0f)); // Usually mvForward
	mpPushBody->AddForceAtPosition(vForce, mpPushBody->GetLocalPosition());
	mlMoveCount = 20;
...
{
	cVector3f vForce = (mpPlayer->GetCharacterBody()->GetRight() * (afMul * 4.0f)); // Usually mvRight
	mpPushBody->AddForceAtPosition(vForce, mpPushBody->GetLocalPosition()); // Usually mvPickPoint
	mlMoveCount = 20;
...
{ 
	...
	{
		cVector3f vForce = (mpPlayer->GetCharacterBody()->GetRight() * (afVal * 100.0f * mpPlayer->mfRightMul));
		cVector3f vBodyCentre = cMath::MatrixMul(mpPushBody->GetLocalMatrix(), mpPushBody->GetMassCentre());
		cVector3f vJointToBody = cMath::Vector3Normalize(vBodyCentre - mpPushBody->GetJoint(0)->GetPivotPoint());
		if (mpPushBody->GetEntityType() == "Object")
			vJointToBody += mpPushBody->GetJoint(0)->GetPinDir();
		//cVector3f vPosition = mpPushBody->GetJoint(0)->GetPivotPoint() + mpPushBody->GetJoint(0)->Get;
		//WITHIN_THE_LINES(vPosition, mpPushBody->GetShape()->GetSize())
		mpPushBody->AddForceAtPosition(vForce, vBodyCentre);
		mlMoveCount = 20;
	}
	else
	{
...
bool cPlayerState_Move::OnAddPitch(float afVal)
{
	if(std::abs(afVal) > kEpsilonf)
	{
		cVector3f vForce = (mpPlayer->GetCharacterBody()->GetUp() * (-afVal * 100.0f  * mpPlayer->mfUpMul)) + 
			(mpPlayer->GetCharacterBody()->GetForward() * (afVal * -80.0f *mpPlayer->mfForwardUpMul));
		cVector3f vBodyCentre = cMath::MatrixMul(mpPushBody->GetLocalMatrix(), mpPushBody->GetMassCentre());
		cVector3f vJointToBody = cMath::Vector3Normalize(vBodyCentre - mpPushBody->GetJoint(0)->GetPivotPoint());
		//cVector3f vPosition = mpPushBody->GetJoint(0)->GetPivotPoint() + mpPushBody->GetJoint(0)->Get;
		//WITHIN_THE_LINES(vPosition, mpPushBody->GetShape()->GetSize())
...
{	
	...
	if (mfYawPull > 1.0f)
		mfYawPull = LERP(mfYawPull, 1.0f, 0.1f);

	iPhysicsWorld *pPhysicsWorld = mpInit->mpGame->GetScene()->GetWorld3D()->GetPhysicsWorld();
	cVector3f vPlayerPos = mpPlayer->GetCharacterBody()->GetPosition() + (vPosAdd / cMath::Max(1.0f, mfYawPull));
...
bool cPlayerState_Push::OnMoveForwards(float afMul, float afTimeStep)
{
	...
	if(fSpeed < mfMaxSpeed)
	{
		...
		{
			cVector3f vForce = mpPlayer->GetCharacterBody()->GetForward() * afMul*100.0f;//*0.5f; Was mvForward
...
bool cPlayerState_Push::OnMoveSideways(float afMul, float afTimeStep)
{
	...
	if(vVel.Length() < mfMaxSpeed)
	{
		...
		cVector3f vForce = mpPlayer->GetCharacterBody()->GetRight() * afMul*100.0f;//*0.5f; Was mvRight
...
bool cPlayerState_Push::OnAddYaw(float afVal)
{
	if (std::abs(afVal) > kEpsilonf)
	{
		mfYawPull = afVal * 10000.0f * mpPlayer->mfRightMul;
		cVector3f vForce = (mpPlayer->GetCharacterBody()->GetRight() * mfYawPull);
		mpPushBody->AddForce(vForce);
		return true;
	}
