...
#define CENTRAL(x, y, a) : if (x > y) {x -= a; if (x < y) x = y;} else if (x < y) {x += a; if (x > y) x = y;} // Centremann :)

 // Centering a float back to y, by amount a.
inline void centreMann(float &x, float y, float a) {
	if (x > y) {
		x -= a;
		if (x < y)
			x = y;
...
void cPlayerHeadMove::Update(float afTimeStep)
{
	...
	///////////////////////////////
	//Head movement
	centreMann(mfMaxHeadMoveSmooth, mfMaxHeadMove, mfHeadMoveBackSpeed * afTimeStep);
	if(mbActive)
	{
		mbWasActive = true;
		...
		*/
		mfPos = (sin(mfAdd) * mfMaxHeadMoveSmooth) * mpPlayer->GetHeadMoveSpeedMul();
		mfAdd += mfHeadMoveSpeed * afTimeStep * 10.0f;
		if (sin(mfAdd) <= 0.0f)
		{
			mfAdd = 0.0f;
			if (mfFootStepMul > 0)
	...
	else
	{
		...
		centreMann(mfAdd, 0.0f, mfHeadMoveBackSpeed * afTimeStep);
		...
		if(mpInit->mpPlayer->GetPower() < 3.0f)
		{
			if(mfFlickerTime <= 0)
			{
				if(pHudModel->mvLights[0]->IsVisible())
				{
					...
					pSoundHandler->PlayGui("item_flashlight_flicker",false,1);
				}
				else
				{
...
void cPlayerFlashLight::SetActive(bool abX)
{
	...
	if(abX && (mpInit->mpPlayer->GetPower()==0 || mbDisabled)) {
		pSoundHandler->PlayGui("item_flashlight_nopower",false,1);
		return;
	}
...
bool cPlayerHidden::OnIntersect(iPhysicsBody *pBody,cPhysicsRayParams *apParams)
{
	...
	if(pEntity && pEntity->GetMeshEntity())
	{
		...
		for(int i=0; i< pMeshEntity->GetSubMeshEntityNum(); ++i)
		{
			...
			if(pMaterial && 
				(pMaterial->IsTransparent()==false && pMaterial->HasAlpha()==false))
			{
				bFoundSolid = true;
				break;
			}
